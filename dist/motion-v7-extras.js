...
(()=>{
 ...
 host.prepend(layer);
 let symbolAnimations=[],fieldSize='';
 ...
 function rebuildSymbols(){
  ...
  symbolAnimations.forEach(a=>a.cancel());symbolAnimations=[];
  ...
  symbolAnimations=[...layer.querySelectorAll('.symbol-field text')].map((el,i)=>{
   const alpha=Number(el.getAttribute('opacity')),phase=Number(el.dataset.phase),dx=(4+seed(i,8)*2)/scale,dy=(2.5+seed(i,4)*2)/scale,stretch=1.04+seed(i,6)*.09;
